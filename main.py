...
async def root():
    ...
    return {
        ...
        "version": "1.0.0",
        "endpoints": {
            "/process": "POST - Форматирование ВКР",
...
async def validate_requirements(requirements: UploadFile = File(...)):
    ...
    if not requirements.filename.endswith(('.docx', '.doc')):
        raise HTTPException(status_code=400, detail="Файл требований должен быть в формате .docx или .doc")
    ...
    try:
        ...
        try:
            ...
            validated_requirements = formatter.validate_formatting(requirements_json)
        ...
        raise HTTPException(status_code=500, detail=f"Ошибка при обработке требований: {str(e)}")

@app.post("/process")
async def process_docs(vkr: UploadFile = File(...), requirements: UploadFile = File(...)):
    """
    Основная функция обработки ВКР
    """
    ...
    if not vkr.filename.endswith(('.docx', '.doc')):
        raise HTTPException(status_code=400, detail="Файл ВКР должен быть в формате .docx или .doc")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            logger.info(f"Начинаем обработку файлов: ВКР={vkr.filename}, Требования={requirements.filename}")
            ...
            logger.info(f"Требования извлечены: {json.dumps(requirements_json, ensure_ascii=False, indent=2)}")
            
            # Шаг 2: Создаем форматтер и валидируем требования
            formatter = VKRFormatter()
            validated_requirements = formatter.validate_formatting(requirements_json)
            logger.info(f"Требования валидированы: {json.dumps(validated_requirements, ensure_ascii=False, indent=2)}")
            
            # Шаг 3: Применяем форматирование
            logger.info("Применяем форматирование...")
            success = formatter.apply_formatting(str(vkr_path), validated_requirements, str(out_path))
            
            if not success:
                processing_stats["failed"] += 1
                raise HTTPException(status_code=500, detail="Ошибка при применении форматирования")
            
            # Получаем статистику обработки
            format_stats = formatter.get_stats()
            logger.info(f"Форматирование завершено. Статистика: {format_stats}")
            
            # Проверяем, что выходной файл создан
            if not Path(out_path).exists():
                processing_stats["failed"] += 1
                raise HTTPException(status_code=500, detail="Отформатированный файл не был создан")
...
async def process_with_custom_requirements(
    ...
    if not custom_requirements:
        raise HTTPException(status_code=400, detail="Требования должны быть переданы")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            formatter = VKRFormatter()
            validated_requirements = formatter.validate_formatting(custom_requirements)
            success = formatter.apply_formatting(str(vkr_path), validated_requirements, str(out_path))
            ...
            logger.error(f"Ошибка при обработке с кастомными требованиями: {str(e)}")
