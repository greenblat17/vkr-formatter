...
async def get_default_requirements():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Ошибка получения требований: {str(e)}")
...
async def format_vkr(
    ...
    requirements: Optional[UploadFile] = File(
        default=None, description="Файл требований (опционально, используется заглушка)")
):
    """
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            logger.info(
                f"📄 Входной файл: {vkr_path} (существует: {vkr_path.exists()})")
            ...
            logger.info(
                f"📊 Результат форматирования: success={success}, stats={format_stats}")
            ...
            # Проверяем результат
            logger.info(
                f"🔍 Проверяем существование выходного файла: {output_path}")
            ...
            if not output_path.exists():
                ...
                logger.error(
                    f"📂 Содержимое временной директории: {list(tmpdir_path.iterdir())}")
                raise HTTPException(
                    status_code=500,
            ...
            logger.info(
                f"🎉 Форматирование завершено успешно: {output_filename}")
